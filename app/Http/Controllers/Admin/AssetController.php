...
class AssetController extends Controller
{
    public function index(Request $request): View
    {
        $assets = Asset::query()
            ...
            ->when(
                ...
                fn ($query, string $condition) => $query->where('condition', $condition),
            )
            ->latest()
            ->paginate(12)
            ->withQueryString();
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data['slug'] = Str::slug($data['code'].'-'.$data['name']).'-'.Str::lower(Str::random(5));
    ...
    public function label(Request $request, Asset $asset): View
    {
        ...
        $labelSize = in_array($requestedSize, [50, 60, 80, 100], true)
            ? $requestedSize
            : 60;

        return view('admin.assets.label', compact('asset', 'labelSize'));
    }
    ...
    private function validated(Request $request, ?Asset $asset = null): array
    {
        return $request->validate([
            ...
            'condition' => ['required', Rule::in(['Baik', 'Perlu Perbaikan', 'Rusak', 'Hilang'])],
